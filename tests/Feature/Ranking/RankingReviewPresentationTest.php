...
class RankingReviewPresentationTest extends TestCase
{
    ...
    public function test_series_surfaces_expose_the_default_window_and_optional_shortcut(): void
    {
        ...
        $this->assertStringContainsString('Ranking progress', $home);
        $this->assertStringContainsString('Your current stage and the available next actions are shown below.', $home);
        $this->assertStringContainsString("\$step < \$seriesWorkflowStep => 'Done'", $home);
        $this->assertStringContainsString("\$step === \$seriesWorkflowStep => 'Current'", $home);
        $this->assertStringContainsString("default => 'Next'", $home);
        $this->assertStringContainsString('Then mark it reviewed below.', $home);
        $this->assertStringContainsString('What to do next:', $home);
        $this->assertStringContainsString('Review Rankings', $home);
        $this->assertStringContainsString('Accept ranking as correct?', $home);
        $this->assertStringContainsString('Yes, Mark as Reviewed', $home);
        $this->assertStringContainsString('rankingLifecycleModal', $home);
        $this->assertStringNotContainsString('window.confirm(button.dataset.confirm)', $home);
