...
[ "$APPROVED_MIGRATIONS_SET" = true ] || fail 'An explicit per-run approved migration list is required; use base64 of "none" when none are pending'
...
cleanup_preflight() { [ -z "$PREFLIGHT_DIR" ] || { rm -f -- "$PREFLIGHT_DIR/deploy.config" "$PREFLIGHT_DIR/target-migrations" "$PREFLIGHT_DIR/approved-migrations" "$PREFLIGHT_DIR/pending-migrations" "$PREFLIGHT_DIR/post-pending-migrations" "$PREFLIGHT_DIR/empty-approval" "$PREFLIGHT_DIR/preflight.php"; rmdir "$PREFLIGHT_DIR" 2>/dev/null || true; }; }
...
git -C "$APP_PATH" merge --ff-only "${EXPECTED_SHA:-origin/main}"
...
readonly LOCKED_EXPECTED_SHA LOCKED_RECONCILE_MASTERS_PAYMENTS LOCKED_REQUESTED_BRANCH
...
if [ -n "$EXPECTED_SHA" ]; then
    [ "$REMOTE_HEAD" = "$EXPECTED_SHA" ] || fail "Deployed commit $REMOTE_HEAD does not match expected commit $EXPECTED_SHA"
fi
