...
(function ($, window, document) {
  ...
  $(document).on('click', '.refundToWallet', function (e) {
    ...
    $.post(api.refundWallet, {
      ...
      var msg = ((_xhr$responseJSON = xhr.responseJSON) === null || _xhr$responseJSON === void 0 ? void 0 : _xhr$responseJSON.message) || 'Refund failed';
      toastr.error(msg);
      logXhrFail('Refund failed', xhr);
    }).always(function () {
      return $btn.removeClass('disabled opacity-50');
    });
  });
  ...
  $('#sendMailForm').on('submit', function (e) {
    ...
    $.post(api.sendMail, $(this).serialize()).done(function (res) {
      ...
      var _xhr$responseJSON2, _xhr$responseJSON3, _xhr$responseJSON3$re;
      var msg = ((_xhr$responseJSON2 = xhr.responseJSON) === null || _xhr$responseJSON2 === void 0 ? void 0 : _xhr$responseJSON2.message) || ((_xhr$responseJSON3 = xhr.responseJSON) === null || _xhr$responseJSON3 === void 0 ? void 0 : (_xhr$responseJSON3$re = _xhr$responseJSON3.result) === null || _xhr$responseJSON3$re === void 0 ? void 0 : _xhr$responseJSON3$re.message) || 'Failed to send email';
    ...
    $.get(api.loadRoster, {
      ...
      slots.forEach(function (slot) {
        ...
        players.forEach(function (p) {
          var selected = parseInt(slot.player_id) === parseInt(p.id) ? 'selected' : '';
          html += "<option value=\"".concat(p.id, "\" ").concat(selected, ">\n              ").concat(p.surname, ", ").concat(p.name, "\n            </option>");
        });
        var paid = parseInt(slot.pay_status) === 1;
        html += "\n                </select>\n              </td>\n              <td class=\"text-center\">\n                <span class=\"badge ".concat(paid ? 'bg-label-success' : 'bg-label-danger', "\">\n                  ").concat(paid ? 'Paid' : 'Unpaid', "\n                </span>\n              </td>\n            </tr>\n          ");
      });
      ...
      $('#replaceRosterModalBody .roster-player-select').each(function () {
        ...
        $sel.select2({
          ...
          matcher: function matcher(params, data) {
            ...
            var terms = params.term.trim().toLowerCase().split(/\s+/);
