@php
  ...
  $isPaymentUpdate = ($kind ?? 'invitation') === 'custom_payment_update';
  $isPaidUpdate = ($kind ?? 'invitation') === 'custom_paid_update';
@endphp
<div style="width:100%;margin:0;background:#f4f3f8;padding:28px 12px;box-sizing:border-box;font-family:Arial,Helvetica,sans-serif;color:#4f4b5f;line-height:1.55;word-wrap:break-word">
  <div style="width:100%;max-width:640px;margin:0 auto;background:#fff;border:1px solid #e6e3ed;border-radius:12px;overflow:hidden;box-sizing:border-box">
    <div style="background:#15324b;padding:24px;text-align:center"><img src="{{ $logo }}" alt="Cape Tennis" width="150" style="background:#fff;border-radius:8px;padding:7px"><div style="color:#fff;text-transform:uppercase;letter-spacing:1.5px;margin-top:12px">{{ $isPaidUpdate ? 'Confirmed team place update' : ($isPaymentUpdate ? 'Team payment update' : (($kind ?? 'invitation') === 'replacement' ? 'Replacement team invitation' : 'Platteland team invitation')) }}</div></div>
    <div style="padding:30px 32px;box-sizing:border-box;overflow-wrap:anywhere;word-break:break-word">
      ...
      @if(!$isPaidUpdate && ($responseDeadline || $paymentDeadline))<div style="background:#f5f3ff;border:1px solid #ddd6fe;border-radius:8px;padding:14px 18px;margin:18px 0"><strong>{{ $isPaymentUpdate ? 'Your payment deadline' : (($kind ?? 'invitation') === 'replacement' ? 'Your replacement deadline' : 'Your deadlines') }}</strong>@if($isPaymentUpdate) @if($paymentDeadline)<div style="margin-top:7px">Complete payment by: <strong>{{ $paymentDeadline->format('d M Y H:i') }}</strong></div>@endif @elseif(($kind ?? 'invitation') === 'replacement') @if($responseDeadline)<div style="margin-top:7px">Respond and complete payment by: <strong>{{ $responseDeadline->format('d M Y H:i') }}</strong></div>@endif<div style="font-size:12px;color:#6b6475;margin-top:6px">This replacement deadline is the later of the campaign deadline or 24 hours from this invitation, unless the event starts sooner.</div>@if($shortReplacementWindow)<div style="font-weight:bold;color:#b54747;margin-top:7px">The event starts in less than 24 hours. Please respond as soon as possible.</div>@endif @else @if($responseDeadline)<div style="margin-top:7px">Respond by: <strong>{{ $responseDeadline->format('d M Y H:i') }}</strong></div>@endif @if($paymentDeadline)<div>Complete payment by: <strong>{{ $paymentDeadline->format('d M Y H:i') }}</strong></div>@endif @endif</div>@endif
      @if(!$isPaidUpdate)<div style="border:1px solid #d9e5eb;border-radius:8px;padding:16px 18px;margin:22px 0"><strong>{{ $isPaymentUpdate ? 'Complete your payment' : 'How to register' }}</strong>@if($isPaymentUpdate)<p style="margin:10px 0 0">Your place has been accepted. Open the event registration page below to complete the outstanding payment before the deadline.</p>@else<ol style="margin:10px 0 0;padding-left:20px"><li>Sign in to a Cape Tennis account.</li><li>Open the event registration page using the green button below.</li><li>Choose <strong>Register</strong> next to the selected player and complete payment before the deadline.</li><li>Your team place is secured only after Cape Tennis verifies the payment.</li></ol><p style="margin:10px 0 0">If you are unavailable, let us know promptly so the next reserve can be invited.</p>@endif</div>@endif
      <div style="text-align:center;margin:26px 0 10px">@if($publicEventUrl)<a href="{{ $publicEventUrl }}" style="display:inline-block;background:#fff;color:#425466;text-decoration:none;font-weight:bold;border:1px solid #b8c5cf;border-radius:6px;padding:11px 18px;margin:0 4px 8px">View event</a>@endif@if($isPaidUpdate)<a href="{{ $responseUrl }}" style="display:inline-block;background:#16876f;color:#fff;text-decoration:none;font-weight:bold;border:1px solid #16876f;border-radius:6px;padding:11px 18px;margin:0 4px 8px">View invitation</a>@else<a href="{{ $registrationUrl }}" style="display:inline-block;background:#16876f;color:#fff;text-decoration:none;font-weight:bold;border:1px solid #16876f;border-radius:6px;padding:11px 18px;margin:0 4px 8px">{{ $isPaymentUpdate ? 'Complete payment' : 'Register and pay' }}</a>@if(!$isPaymentUpdate)<a href="{{ $responseUrl }}?action=decline" style="display:inline-block;background:#fff;color:#b54747;text-decoration:none;font-weight:bold;border:1px solid #d9a6a6;border-radius:6px;padding:11px 18px;margin:0 4px 8px">Decline invitation</a>@endif@endif</div>
      @if(!$isPaymentUpdate && !$isPaidUpdate)<p style="font-size:13px;color:#777180;text-align:center">Register opens the event page. Decline opens a confirmation window with an optional reason.</p>@endif
