    ...
    <div class="col-md-7 col-lg-6">
      ...
      <div class="card shadow-sm border-primary">
        <div class="card-header bg-primary text-white">
          <h5 class="mb-0"><i class="ti ti-building-bank me-2"></i>Bank Details for Refund{{ $registrations->count() > 1 ? 's' : '' }}</h5>
        </div>
        <div class="card-body">

          {{-- Summary of all pending refunds --}}
          <div class="alert alert-info mb-4">
            <strong>Hi {{ $user->name }},</strong><br>
            Please provide your bank details to receive the following refund{{ $registrations->count() > 1 ? 's' : '' }}:
            <ul class="mb-1 mt-2">
              @foreach($registrations as $reg)
                ...
                @endphp
                <li>
                  <strong>{{ $pName }}</strong> — {{ $eventName }}{{ $catName ? ' (' . $catName . ')' : '' }}
                  <span class="fw-bold text-success">R{{ number_format($reg->refund_net, 2) }}</span>
                </li>
              @endforeach
            </ul>
            @if($registrations->count() > 1)
              <strong>Total: R{{ number_format($registrations->sum('refund_net'), 2) }}</strong>
            @endif
          </div>
          ...
          <form method="POST" action="{{ request()->fullUrl() }}">
            ...
            <div class="mb-3">
              ...
              <input type="text" name="refund_account_name" class="form-control @error('refund_account_name') is-invalid @enderror"
                     value="{{ old('refund_account_name', $registrations->first()->refund_account_name) }}"
            ...
            <div class="mb-3">
              ...
              <select name="refund_bank_name" class="form-select @error('refund_bank_name') is-invalid @enderror" required>
                ...
                @foreach($bankNames as $value => $label)
                  <option value="{{ $value }}" {{ old('refund_bank_name', $registrations->first()->refund_bank_name) === $value ? 'selected' : '' }}>
                    {{ $label }}
                  </option>
                @endforeach
              </select>
            ...
            <div class="mb-4">
              ...
              <select name="refund_account_type" class="form-select @error('refund_account_type') is-invalid @enderror" required>
                ...
                <option value="current" {{ old('refund_account_type', $registrations->first()->refund_account_type) === 'current' ? 'selected' : '' }}>Current</option>
                <option value="savings" {{ old('refund_account_type', $registrations->first()->refund_account_type) === 'savings' ? 'selected' : '' }}>Savings</option>
              </select>
            ...
            <button type="submit" class="btn btn-primary w-100">
              ...
              Submit Bank Details for {{ $registrations->count() === 1 ? 'Refund' : $registrations->count() . ' Refunds' }}
            </button>
          </form>
