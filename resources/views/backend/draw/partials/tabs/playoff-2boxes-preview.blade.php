...
<body>
    ...
    <script>
        ...
        const boxLetters = Object.keys(seededPlayers)
            .map(code => code.charAt(0))
            .filter((v, i, a) => a.indexOf(v) === i);

        const basePattern = ['A', 'B', 'A', 'B', 'A', 'B', 'A', 'B'];
        const positionPattern = [1, 4, 3, 2, 2, 3, 4, 1];

        for (let boxIndex = 0; boxIndex < boxLetters.length; boxIndex += 2) {
            const boxA = boxLetters[boxIndex];
            const boxB = boxLetters[boxIndex + 1];
            if (!boxB) break;

            const getBoxPlayers = (box) => Object.entries(seededPlayers)
                .filter(([code, _]) => code.startsWith(box))
            ...
            const playersA = getBoxPlayers(boxA);
            const playersB = getBoxPlayers(boxB);
            const maxSeeds = Math.max(playersA.length, playersB.length);
            const numDraws = Math.ceil(maxSeeds / 4);

            for (let i = 0; i < numDraws; i++) {
                const offset = i * 4;
                const draw = basePattern.map((letter, j) => {
                    const pos = positionPattern[j] + offset;
                    const code = `${letter === 'A' ? boxA : boxB}${pos}`;
                    const player = [...playersA, ...playersB].find(p => p.code === code);
        ...
        function buildAllDraws(drawSets, showNames = false) {
            ...
            for (let i = 0; i < drawSets.length; i++) {
                const yOffset = i * 900;
                const players = showNames ? drawSets[i] : drawSets[i].map(name => (name === 'Bye' ? 'Bye' : 'TBD'));
