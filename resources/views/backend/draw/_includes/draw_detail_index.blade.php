<div class="card m-2">
  ...
  <a href="{{ route('backend.individual-schedule.page', $draw->id) }}"
...
{{-- Inject venue route patterns --}}
<script>
    window.venueStoreBase = "{{ route('backend.draw.venues.store', ['draw' => 'DRAW_ID']) }}";
    window.venueJsonBase = "{{ route('backend.draw.venues.json', ['draw' => 'DRAW_ID']) }}";
    window.allVenuesUrl = "{{ route('venue.list') }}";
</script>
