      ...
      <div class="masters-entry-table">
        <div class="masters-entry-head"><span>#</span><span>Player</span><span>Email</span><span>Cell</span><span>Status / payment</span><span>Action</span></div>
          @forelse($invitees as $playerInvitation)
            ...
            @php($playerStatus = match ($playerInvitation->status) { \App\Models\MastersInvitation::PAID_CONFIRMED => 'Registered', \App\Models\MastersInvitation::ACCEPTED_PENDING_PAYMENT => 'Payment pending', default => 'Invitation will be sent' })
    ...
    form.addEventListener('submit', async function (event) {
      ...
      try {
        ...
        if (description && description.classList.contains('player-status')) {
          description.textContent = invited ? 'Invitation will be sent' : 'Reserve — invite if needed';
        }
