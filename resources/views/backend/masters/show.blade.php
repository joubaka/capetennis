    ...
    <div class="card-body">
      ...
      @if($declined->isNotEmpty())<details class="mt-3 masters-declined"><summary class="masters-declined-summary small fw-semibold text-danger">Declined / unavailable · {{ $declined->count() }} player{{ $declined->count() === 1 ? '' : 's' }}</summary><div class="small text-muted mb-1">The next reserve can be invited when automatic replacement is enabled.</div><div class="masters-player-list masters-declined-panel">
          @foreach($declined as $playerInvitation)
            @php($contactEmail = $playerInvitation->player?->email ?: $playerInvitation->player?->user?->email ?: $playerInvitation->player?->users?->first()?->email)
            <div class="masters-player-row d-flex align-items-start gap-2 bg-light"><div class="flex-grow-1"><strong>{{ $playerInvitation->player?->full_name ?? ('Player '.$playerInvitation->player_id) }}</strong><div class="small text-muted">Rank {{ $playerInvitation->ranking_position }} · {{ $playerInvitation->status === \App\Models\MastersInvitation::WITHDRAWN ? 'Withdrawn after registration' : ($playerInvitation->status === \App\Models\MastersInvitation::ADMIN_REMOVED ? 'Removed by admin' : 'Declined / unavailable') }}</div><div class="small mt-1 d-flex flex-wrap gap-2"><span class="text-muted">Contact:</span>@if($contactEmail)<a href="mailto:{{ $contactEmail }}">{{ $contactEmail }}</a>@else<span class="text-muted">No email</span>@endif @if($playerInvitation->player?->cellNr)<a href="tel:{{ $playerInvitation->player->cellNr }}">{{ $playerInvitation->player->cellNr }}</a>@else<span class="text-muted">No telephone</span>@endif</div>@if($playerInvitation->status === \App\Models\MastersInvitation::DECLINED)<div class="small text-danger mt-2"><strong>Player decline</strong> · {{ $playerInvitation->declined_at?->format('d M Y H:i') ?? 'time unavailable' }}@if($playerInvitation->declinedBy) · {{ $playerInvitation->declinedBy->name ?? $playerInvitation->declinedBy->email }}@endif · {{ str_replace('_', ' ', ucfirst($playerInvitation->decline_method ?? 'recorded')) }}@if($playerInvitation->decline_confirmed_at) · confirmed {{ $playerInvitation->decline_confirmed_at->format('d M Y H:i') }}@endif@if($playerInvitation->decline_reason) · “{{ $playerInvitation->decline_reason }}”@endif</div>@elseif($playerInvitation->status === \App\Models\MastersInvitation::ADMIN_REMOVED)<div class="small text-warning mt-2"><strong>Admin removal</strong> · {{ $playerInvitation->admin_removed_at?->format('d M Y H:i') ?? $playerInvitation->declined_at?->format('d M Y H:i') ?? 'time unavailable' }}@if($playerInvitation->adminRemovedBy) · {{ $playerInvitation->adminRemovedBy->name ?? $playerInvitation->adminRemovedBy->email }}@endif · admin dashboard</div>@endif</div><span class="badge bg-label-danger">{{ $playerInvitation->status === \App\Models\MastersInvitation::WITHDRAWN ? 'Withdrawn' : ($playerInvitation->status === \App\Models\MastersInvitation::ADMIN_REMOVED ? 'Admin removed' : 'Player declined') }}</span></div>
          @endforeach
